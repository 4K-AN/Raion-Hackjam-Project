...
public class SequenceManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        foreach (var pair in player1KeyMap) 
            if (!keySpriteMap.ContainsKey(pair.key)) keySpriteMap.Add(pair.key, pair.sprite);
    ...
    {
        if (p1SequenceContainer == null || keyIconPrefab == null || player1KeyMap == null || player1KeyMap.Count == 0) return;

        // Aktifkan container
        p1SequenceContainer.gameObject.SetActive(true);
    ...
    {
        if (p2SequenceContainer == null || keyIconPrefab == null || player2KeyMap == null || player2KeyMap.Count == 0) return;

        // Aktifkan container
        p2SequenceContainer.gameObject.SetActive(true);
    ...
    public bool ProcessInput(int playerID, Key key)
    {
        if (playerID == 1 && p1Active)
        {
            ...
            {
                if (p1Icons != null && p1Index < p1Icons.Count) p1Icons[p1Index].color = correctColor;
                p1Index++;
                if (p1Index >= sequenceLength) WinClash(1);
            }
            else ResetPlayerSequence(1);
            return true;
        }
        else if (playerID == 2 && p2Active)
        {
            ...
            {
                if (p2Icons != null && p2Index < p2Icons.Count) p2Icons[p2Index].color = correctColor;
                p2Index++;
                if (p2Index >= sequenceLength) WinClash(2);
            }
            else ResetPlayerSequence(2);
            return true;
        }
        return false;
    }
    ...
    private void ResetPlayerSequence(int playerID)
    {
        ...
        {
            p1Index = 0;
            if (p1Icons != null) foreach (var icon in p1Icons) if (icon != null) icon.color = pendingColor;
        }
        else if (playerID == 2)
        {
            p2Index = 0;
            if (p2Icons != null) foreach (var icon in p2Icons) if (icon != null) icon.color = pendingColor;
        }
    }
    ...
    private void DisplaySequenceIcons(List<Key> sequence, Transform container, ref List<Image> iconList)
    {
        ...
        foreach (Key k in sequence)
        {
            ...
            if (img != null && keySpriteMap.TryGetValue(k, out Sprite sp))
            {
                img.sprite = sp;
    ...
    public void ClearSequences()
    {
        ...
        {
            foreach (Transform child in p1SequenceContainer) Destroy(child.gameObject);
            p1SequenceContainer.gameObject.SetActive(false);
        }

        if (p2SequenceContainer != null)
        {
            foreach (Transform child in p2SequenceContainer) Destroy(child.gameObject);
            p2SequenceContainer.gameObject.SetActive(false);
        }
